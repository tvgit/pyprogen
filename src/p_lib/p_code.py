...
def p_inform_about_paths_and_filenames():
    ...
    print '-'*10 + ' ' + p_glbls.prog_name + ' ' + '-' *60
    len_dir_main = len(p_glbls.dir_main) + 1
    print 'Path of YOUR code is:             ' + os.path.join(p_glbls.dir_main, p_glbls.prog_name)
    print 'Filename of YOUR code is:         ' + ' '*len_dir_main + p_glbls.prog_name
    print 'You will find the new version in: ' + os.path.join(p_glbls.dir_main, '')
    print 'Path of new version is:           ' + os.path.join(p_glbls.dir_main, p_glbls.prog_name_act_cfg)
    print 'Filename of new version is:       ' + ' '*len_dir_main + p_glbls.prog_name_act_cfg
    # print '(>' + p_glbls.prog_name + '< was not changed.)'
    # cfg_path_tmp
    len_cfg_dir = len(os.path.dirname(p_glbls.cfg_path_tmp)) + 1
    print 'Path of new cfg-file is:          ' + p_glbls.cfg_path_tmp
    print 'Filename of new cfg-file is:      ' + ' '*len_cfg_dir + os.path.basename(p_glbls.cfg_path_tmp)
    # print ' The new files have a timestamp  >' + p_glbls.prog_name[:-3] + '_YYYY_MM_DD-hh_mm_ss.py<.'
    print
    print 'You may configure the comand line _args_     of >' + p_glbls.prog_name + '<  via:  >new_prog_args.cfg<'
    print ' ... but run >pyprogen.py< again!'
    print 'You may configure the comand line _defaults_ of >' + p_glbls.prog_name + '<  via:  >' + cfg_path + '<'
    ...
    print
    print 'Do not change the >' + p_glbls.dir_lib + '\*.py< files!\n'
    print '-'*10 + ' ' + p_glbls.prog_name + ' ' + '-' *60
    print
...
def p_main_was_modified(outfile_path):
    ...
    if old_hash_str != hash_of_code:         # hashes are identical?
        ...
        p_glbls.prog_changed = True
        return True
    else:
        p_log_this ('>' + outfile_path + '<  + is unchanged')
        p_glbls.prog_changed = False
        return False
...
def p_create_main():
    ...
    if p_main_was_modified(outfile_path):
        ...
        p_glbls.prog_name_act_cfg = os.path.basename(outfile_path)
