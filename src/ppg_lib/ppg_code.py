...
def p_read_ini(dir_cfg='.', cfg_fn='new_prog.ini'):
    ...
    ppg_glbls.batch_name = ppg_glbls.main_name[:-3] + '.bat'
...
def p_batch_make_code():
    ...
    txt += "pyinstaller.exe --onefile xx_main.py"
...
def p_pyinstaller_make():
    # make bat-file: "pyinstaller.exe --onefile w_main.py":
    p_log_this('creating: ' + ppg_glbls.batch_name)
    code_lines = p_batch_make_code()
    # p_write_code(code_dict, ppg_glbls.code_new_path)
    p_write_code(code_lines, ppg_glbls.code_new_path)
