...
def p_read_ini(dir_cfg='.', cfg_fn='new_prog.ini'):
    ...
    # ppg_glbls.batch_name & ppg_glbls.main_path
    ppg_glbls.batch_name = ppg_glbls.main_name[:-3] + '_pyinstaller.bat'
    ppg_glbls.batch_path = os.path.join(ppg_glbls.main_dir, ppg_glbls.batch_name)
...
def p_batch_make_code():
    # create code batch file;
    txt  = r"pyinstaller.exe --onefile xx_main.py"
    txt += "\n"
    txt += r"xcopy .\cfg .\dist\cfg /s /e /h /I /y"
    txt += "\n"
    txt += r"xcopy .\DataIn .\dist\DataIn /s /e /h /I /y"
    txt += "\n"
    txt += r"xcopy .\DataOut .\dist\DataOut /s /e /h /I /y"
    txt += "\n"
    txt += r"mkdir .\dist\log"
    txt += "\n"
    txt += "\n"
    txt += r"REM windows is interesting: delete a folder recursively:"
    txt += "\n"
    txt += r"mkdir .\void"
    txt += "\n"
    txt += r"robocopy .\void .\build /MIR"
    txt += "\n"
    txt += r"rmdir .\void /S /Q"
    txt += "\n"
    txt += r"rmdir .\build /S /Q"
    txt += "\n"

    # add txt to batch file.
    patterns.y_pyinst[10] = txt
    # substitute x_main to real name.
    patterns.y_pyinst = p_subst_vars_in_patterns(patterns.y_pyinst)

    code_lines = ''
    for key, line in sorted(patterns.y_pyinst.iteritems()):
        code_lines += line
    return code_lines
...
def p_pyinstaller_make():
    # make bat-file: "pyinstaller.exe --onefile y_main.py":
    p_log_this('creating: ' + ppg_glbls.batch_path)

    code_lines = p_batch_make_code()

    code_dict = dict()  # p_write_code wants dict as input
    txt  = ('REM ' + ppg_glbls.date_time_str + ' generated by: >pyprogen.py<\n')
    code_dict[0] = txt
    code_dict[1] = code_lines

    patterns.y_main = p_subst_vars_in_patterns(patterns.y_main)


    p_write_code(code_dict, ppg_glbls.batch_path)
